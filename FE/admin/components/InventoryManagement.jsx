...
import { Package, Plus, ClipboardList, Trash2, Save, FileText, Edit2 } from 'lucide-react';
...
export default function InventoryManagement() {
  ...
  const handleAddItemSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await axiosClient.put(`/admin/Kho/nguyen-lieu/${editingItem.maNL}`, newItem);
  ...
  const handleDeleteItem = async (maNL) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa nguyên liệu này?")) {
      try {
        await axiosClient.delete(`/admin/Kho/nguyen-lieu/${maNL}`);
        fetchInventory();
      } catch (error) {
        const errorMsg = error.response?.data?.message || "Không thể xóa nguyên liệu này!";
        alert("⚠️ Lỗi từ Backend:\n" + errorMsg);
      }
    }
  };
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: '2rem' }}>
      ...
      {/* BẢNG TỒN KHO */}
      <div className="glass-panel" style={{ backgroundColor: 'white', borderRadius: '16px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', overflowX: 'auto', height: '700px', width: '960px' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ backgroundColor: '#f3f4f6', borderBottom: '2px solid #e5e7eb' }}>
              ...
              <th style={{ padding: '1.25rem 1rem', color: '#374151', fontWeight: 'bold', textAlign: 'center' }}>Thao tác</th>
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={item.maNL || index} style={{ borderBottom: '1px solid #e5e7eb' }}>
                ...
                <td style={{ padding: '1rem', textAlign: 'center' }}>
                  <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                    <button onClick={() => handleOpenItemModal(item)} style={{ padding: '0.4rem 0.6rem', backgroundColor: '#eff6ff', color: '#3b82f6', border: '1px solid #bfdbfe', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.25rem', fontWeight: '500' }}>
                      <Edit2 size={16} /> Sửa
                    </button>
                    <button onClick={() => handleDeleteItem(item.maNL)} style={{ padding: '0.4rem 0.6rem', backgroundColor: '#fef2f2', color: '#ef4444', border: '1px solid #fecaca', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.25rem', fontWeight: '500' }}>
                      <Trash2 size={16} /> Xóa
                    </button>
                  </div>
                </td>
            ...
            {inventory.length === 0 && (
              <tr>
                <td colSpan="7" style={{ padding: '3rem', textAlign: 'center', color: '#6b7280', fontStyle: 'italic' }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '1rem' }}>
          <div style={{ backgroundColor: 'white', padding: '2rem', borderRadius: '16px', width: '100%', maxWidth: '400px', boxSizing: 'border-box' }}>
            <h3 style={{ marginTop: 0, color: '#111827', fontSize: '1.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Package color="#10b981" /> {editingItem ? 'Chỉnh sửa nguyên liệu' : 'Khai báo nguyên liệu'}
            </h3>

            <form onSubmit={handleAddItemSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.5rem', color: 'black' }}>Mã Nguyên Liệu</label>
                <input placeholder="VD: NL01" value={newItem.maNL} onChange={e => setNewItem({ ...newItem, maNL: e.target.value })} disabled={!!editingItem} required style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid black', boxSizing: 'border-box', backgroundColor: 'white', color: 'black' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.5rem', color: 'black' }}>Tên Nguyên Liệu</label>
                <input placeholder="VD: Thịt Bò Úc" value={newItem.tenNL} onChange={e => setNewItem({ ...newItem, tenNL: e.target.value })} required style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid black', boxSizing: 'border-box', color: 'black' }} />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.5rem', color: 'black' }}>Đơn vị tính</label>
                  <input placeholder="VD: Kg, Lít..." value={newItem.donViTinh} onChange={e => setNewItem({ ...newItem, donViTinh: e.target.value })} required style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid black', boxSizing: 'border-box' }} />
                </div>
                {/* THÊM Ô NHẬP ĐƠN GIÁ CHUẨN */}
                <div>
                  <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.5rem', color: 'black' }}>Đơn giá (VNĐ)</label>
                  <input type="number" min="0" step="1000" placeholder="VD: 50000" value={newItem.donGia} onChange={e => setNewItem({ ...newItem, donGia: parseFloat(e.target.value) || 0 })} required style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid black', boxSizing: 'border-box' }} />
                </div>
              </div>

              <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                <button type="submit" style={{ flex: 1, backgroundColor: '#10b981', color: 'white', border: 'none', padding: '0.75rem', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>{editingItem ? 'Lưu cập nhật' : 'Lưu vào kho'}</button>
          ...
          <div style={{ backgroundColor: 'white', padding: '2rem', borderRadius: '16px', width: '100%', maxWidth: '800px', maxHeight: '90vh', overflowY: 'auto', boxSizing: 'border-box' }}>
            ...
            <form onSubmit={handleImportSubmit}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.5rem', color: 'black' }}>Nhà cung cấp (Tùy chọn)</label>
                  <input placeholder="Tên nhà cung cấp..." value={importData.nhaCungCap} onChange={e => setImportData({ ...importData, nhaCungCap: e.target.value })} style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid black', boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.5rem', color: 'black' }}>Ghi chú phiếu nhập</label>
                  <input placeholder="Ghi chú..." value={importData.ghiChu} onChange={e => setImportData({ ...importData, ghiChu: e.target.value })} style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid black', boxSizing: 'border-box' }} />
                </div>
              </div>
              ...
              <div style={{ backgroundColor: '#f9fafb', padding: '1rem', borderRadius: '8px', border: '1px solid #e5e7eb', marginBottom: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                  <h3 style={{ margin: 0, fontSize: '1.125rem', color: 'black' }}>Danh sách hàng nhập</h3>
                ...
                {importData.danhSachChiTiet.map((item, index) => (
                  <div key={index} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1.5fr 1.5fr auto', gap: '0.75rem', alignItems: 'end', marginBottom: '1rem' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '0.75rem', color: 'black', marginBottom: '0.25rem' }}>Nguyên liệu</label>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '0.75rem', color: 'black', marginBottom: '0.25rem' }}>Số lượng</label>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '0.75rem', color: 'black', marginBottom: '0.25rem' }}>Đơn giá (VNĐ)</label>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '0.75rem', color: 'black', marginBottom: '0.25rem' }}>Thành tiền</label>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '2px solid #e5e7eb', paddingTop: '1.5rem' }}>
                <div style={{ fontSize: '1.25rem', color: 'black' }}>
