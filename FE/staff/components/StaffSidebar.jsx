...
import { Table, LogOut, Receipt } from 'lucide-react';
...
export default function StaffSidebar() {
  ...
  const menuItems = [
    { path: '/staff/orders', icon: Receipt, label: 'Hóa Đơn (Orders)' },
    { path: '/staff/tables', icon: Table, label: 'Sơ Đồ Bàn (Tables)' },
  ];
    ...
    <aside className="glass-panel" style={{ width: '280px', margin: '1rem', display: 'flex', flexDirection: 'column', borderRadius: '24px', height: 'calc(100vh - 2rem)' }}>
      <div style={{ padding: '2rem', borderBottom: '1px solid var(--border-color)' }}>
        ...
        <p className="text-muted" style={{ fontSize: '0.875rem', marginTop: '0.5rem' }}>
          Xin chào, <span style={{ color: 'var(--text-main)', fontWeight: 'bold' }}>{user?.name || 'Nhân viên'}</span>
        </p>
      </div>
